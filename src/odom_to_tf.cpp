...
public:
  OdomToTF() : Node("odom_to_tf") {
    ...
    odom_topic =
        this->declare_parameter("odom_topic", std::string("/odom/perfect"));
    ...
    if (child_frame_id != "") {
      RCLCPP_INFO(this->get_logger(), "child_frame_id set to %s",
                  child_frame_id.c_str());
    } else {
      RCLCPP_INFO(this->get_logger(),
                  "child_frame_id was not set. The child_frame_id of the odom "
...
private:
  ...
  void odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg) const {
    ...
    if (not inverse_tf) {
      tfs_.header.frame_id = frame_id != "" ? frame_id : msg->header.frame_id;
      tfs_.child_frame_id =
          child_frame_id != "" ? child_frame_id : msg->child_frame_id;
      ...
      tfs_.child_frame_id =
          child_frame_id != "" ? child_frame_id : msg->header.frame_id;
      tf2::Vector3 trans;
